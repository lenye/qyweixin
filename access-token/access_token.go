...
func (p *AccessTokenClient) Loop(appId, appSecret string) {
	...
	beatInterval = time.Duration(newAccessToken.NextGet) * time.Millisecond
	glog.Infof("next access-token beat interval=%v", beatInterval)
	beatTimer := time.NewTimer(beatInterval)
	defer func() {
		if !beatTimer.Stop() {
			<-beatTimer.C
		}
